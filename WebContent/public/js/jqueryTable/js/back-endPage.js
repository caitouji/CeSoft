;(function($, window, document,undefined) {
  ...
  var maxpagenumberBoxRemove,eRemove;
  cjjTable.prototype = {
        start:function(){
            var _this = this;
            var titlelistBox="";
            ...
            var histroy_DDBox = "";
            var histroy_DD = "";
            var firstPageNumber=json.total>_this.options.pageJson.pageSize?_this.options.pageJson.pageSize:json.total;
            for (var i = 0; i <json.list.length; i++) {
                ...
                for(var x=0;x<_this.options.body.length;x++){
                    ...
                    bodyBox+="<td data-html='"+json.list[i][type]+"' class='"+type+"' style='display:"+display+"'>"+json.list[i][type]+"</td>";
                    bodyBigBox = bodyBox;
                }
            ...
            if (Math.ceil(json.total/ _this.options.pageJson.pageSize) == 1) {
            ...
            if (Math.ceil(json.total/ _this.options.pageJson.pageSize) < maxpagenumberBox) {
                for (var i = 0; i < Math.ceil(json.total/ _this.options.pageJson.pageSize); i++) {
                    var  className = "";
                    if(i==0){
            ...
            if(Math.ceil(json.total/_this.options.pageJson.pageSize)<2){
        ...
        inputKeyup:function(e,maxpagenumberBox,json){
            ...
            } else if (val > 0 && val <= Math.ceil(json.total / e.options.pageJson.pageSize)) {
                var val2 = val.replace(/[^0-9]/g, "");
            } else if (val > Math.ceil(json.total/ e.options.pageJson.pageSize)) {
                var val2 = Math.ceil(json.total / e.options.pageJson.pageSize);
            }
        ...
        endPage:function(e,maxpagenumberBox,json){
            $(e.$element.selector+ " .typeNumber").val(Math.ceil(json.total / e.options.pageJson.pageSize));
            e.page( maxpagenumberBox, e);
        },
        ...
        page:function( maxpagenumberBox,e) {
            $(e.$element.selector).find("input[name='checkBoth']").removeAttr('checked');
            var histroy_DDBox = "";
            var histroy_DD = "";
            var pageNUmber = e.options.pageJson.pageSize;
            var that = e.$element;
            ...
            var lastPage=Pagenumber<Math.ceil(json.total / pageNUmber)?Pagenumber*pageNUmber:json.total;
            $(e.$element.selector+ " .typeNumber").val(Pagenumber<Math.ceil(json.total / pageNUmber)?Pagenumber:Math.ceil(json.total / pageNUmber));
            var jsonLength = json.total==0?1:json.total;
            ...
            var maxpagenumberBoxBigbox = "";
            var maxpagenumberBoxBig = "";
            if (Math.ceil(jsonLength/ e.options.pageJson.pageSize) < maxpagenumberBox) {
                for (var i = 0; i < Math.ceil(jsonLength/ e.options.pageJson.pageSize); i++) {
                    var  className = "";
                    if(i==0){
                        className = "pagenumberBoxLi";
                    }
            ...
            if(Math.ceil(jsonLength/e.options.pageJson.pageSize)<2){
        ...
        reload:function(maxpagenumberBox,e){
              e.page(maxpagenumberBox,e);
        }
  }
  $.fn.CJJTable = function(options){
         ...
         $.fn.CJJReload = function(){
            return cjj.reload(maxpagenumberBoxRemove,eRemove);
        }
         return cjj.start();
  }
